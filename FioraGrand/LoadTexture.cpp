...
void LoadTexture::Render(SDL_Renderer* renderer, std::string id,
    int x, int y, int width, int height, SDL_RendererFlip flip_flag)
{
    SDL_Rect src_rect = {0, 0, width, height};
    SDL_Rect dst_rect = {x, y, width, height};
    SDL_RenderCopyEx(renderer, g_texture_map[id],
        &src_rect, &dst_rect, 0, nullptr, flip_flag);
}

void LoadTexture::RenderByFrame(SDL_Renderer* renderer, std::string id,
    int x, int y, int width, int height, int frame, SDL_RendererFlip flip_flag)
{
